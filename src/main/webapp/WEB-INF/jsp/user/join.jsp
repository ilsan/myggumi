...
<style type="text/css">
	...
	#body_table input, select{
		border: 1px solid #ccc;
		background-color: rgb(249,249,249);
		height: 20px;
	}
	#body_table label{
		font-family: Y730,Gulim,"돋움",Dotum,Helvetica,sans-self;
		outline: none;
		color: #333;
	}
	#body_table input[type=checkbox]{
		margin:0 3px 0 0;
	}
	#body_table input[type=button] {
		height:25px;
	}
	#body_table input:focus {
...
<script type="text/javascript">
	...
	var jquerycheck = false;	//서브밋실행전 정규식/빈칸 체크확인용
	var idcheck = false;		//아이디 중복확인/정규식 
	var pwdcheck = false;		//비밀번호 정규식
	var pwdcheck2 = false;		//비밀번호확인칸 비밀번호 같게 입력했는지 확인용
	var addrcheck = false;		//우편찾기 여부 체크
	var mobilecheck = false;	//휴대폰 체크
	
		$(document).ready(function(){
				//중복체크 클릭
			$("#userIdCheck").click(function(){
				var userId = $("#userId").val();
				var patt = new RegExp(/^[a-z0-9]{4,16}$/g);
				//new RegExp(/^[a-z][a-z0-9_]+$/);
				var bool = patt.test(userId);
				if(!bool){
					alert("아이디는 4~16이내 소문자/숫자로만 입력해주세요");
					$("#userId").focus();
				}
				if(bool){
				...
				$.ajax({  // Ajax 시작하겠다는 의미
					...
					success: function(data) {
	 					if(data == 1) { // 아이디가 존재(중복)하는 경우
							alert("중복 아이디입니다 다시 입력하세요");
							$("#userId").val('').focus();
							idcheck=false;
							return false;  
			...
			$("#submit").click(function(){
				...
				//비밀번호 정규식
				var patt = new RegExp(/^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{10,16}$/g);
				var userPwd = $("#userPwd").val();
				var bool = patt.test(userPwd);
				if(bool){
					pwdcheck=true;
					//비밀번호 정규식이 통과된 상태에서
				...
				//우편번호찾기 여부
				var userAddrcheck = $("#userZipCode").val();
				if(userAddrcheck != ""){
					addrcheck = true;
				}
				
				//휴대폰체크
				var mobileptt = new RegExp(/^[0-9]*$/);
				var userMobile2 = $("#userMobile2").val();
				var userMobile3 = $("#userMobile3").val();
				var mobilebool2 =  mobileptt.test(userMobile2);
				var mobilebool3 = mobileptt.test(userMobile3);
				if(mobilebool2 && mobilebool3){
					mobilecheck = true;
				}
		...
		function join_check(){
			if(jquerycheck == true){
				//회원구분체크 필수
				if(document.getElementById('userGroup').checked == false){
					alert("회원구분을 선택해주세요");
				...
				//이용약관 동의 필수
				if(document.getElementById('tos1').checked == false){
					alert("이용약관의 동의해야합니다");
					document.getElementById('tos1').focus();
					idcheck = false;		 
					pwdcheck = false;		
					pwdcheck2 = false;		
					addrcheck = false;		
				...
				//개인정보수집 동의 필수
				if(document.getElementById('tos2').checked == false){
					alert("개인정보수집의 동의해야합니다");
					document.getElementById('tos2').focus();
					idcheck = false;		 
					pwdcheck = false;		
					pwdcheck2 = false;		
					addrcheck = false;		
				...
				//비밀번호 정규식 확인
				if(pwdcheck == false){
					alert("비밀번호는 영문 대소문자/숫자/특수문자포함 10자~16자로 입력해주세요");
				...
				if(addrcheck == false){
					alert("주소를 입력해주세요");
					document.getElementById('find_addr').focus();
					idcheck = false;		 
					pwdcheck = false;		
					pwdcheck2 = false;		
					addrcheck = false;		
				...
				if(mobilecheck == false){
					alert("휴대폰번호는 숫자만입력해주세요")
					document.getElementById('userMobile2').focus();
					idcheck = false;		 
					pwdcheck = false;		
					pwdcheck2 = false;		
					addrcheck = false;		
		...
		function selectEmail(){
			document.getElementById("userEmail2").value = document.getElementById("userEmail3").value;
		}
	...
	<div id="main">
	  <div id="top">
	  	...
	  	<table border="0">
			<tr>
				<td> <span>회원구분</span> 
					<input type="radio" name="userGroup" id="userGroup">개인회원
				</td>			
			</tr>
		</table>
		<hr>
	 ...
	 <div id="body">
			...
			<table border="0">
				...
				<tr>
					...
					<td>
						...
						<input type="button" id="userIdCheck" value="중복체크">
						(영문소문자/숫자,4~16자)
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input type="text" name="userZipCode" readonly id="userZipCode">-<input type="button" onclick="find_address()" value="우편번호 ▶" id="find_addr"><br>
						<input type="text" name="userAddr1" readonly id="userAddr1">기본주소<br>
						<input type="text" name="userAddr2" id="userAddr2" required>나머지주소
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						...
						-
						<input name="userMobile2" maxlength="4" id="userMobile2" required>
						-
						<input name="userMobile3" maxlength="4" id="userMobile3" required>
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<label>
							<input type="checkbox" name="userSmsCheck" value="true">동의함<br>
						</label>
				...
				<tr>
					...
					<td>
						<input name="userEmail1" id="userEmail1" required>
						@
						<input name="userEmail2" id="userEmail2" required>
						<select id="userEmail3" onchange="selectEmail()">
				...
				<tr>
					...
					<td>
						<label>
							<input type="checkbox" name="userEmailCheck" value="true">동의함<br>
						</label>
		...
		<div id="foot">
			...
			<div id="foot_body">
				<hr>
				<div class="foot_body_left">
					이용약관 동의(필수)
				</div><!-- foot_body_left -->
				<div class="foot_body_textarea">
				<textarea rows="50" cols="100" readonly>제1조(목적)
이 약관은 (주)립슨(전자상거래 사업자)이 운영하는 지모 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리?의무 및 책임사항을 규정함을 목적으로 합니다.
※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다」
제2조(정의)
①“몰” 이란 (주)립슨 회사가 재화 또는 용역(이하 “재화등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
부 칙(시행일) 이 약관은 년 월 일부터 시행합니다.
				...
				<br>
					<p style="text-align: right;">이용약관의 동의하십니까?<input type="checkbox" id="tos1">동의함</p> 
				</div><!-- foot_body_textarea -->
				<hr>
				<div class="foot_body_left">
					개인정보 수집 및<br>
 					이용 동의(필수)
				...
				<div class="foot_body_textarea">
					<textarea rows="50" cols="100" readonly>
1. 개인정보 수집목적 및 이용목적
...
※ 동의를 거부할 수 있으나 거부시 회원 가입이 불가능합니다.
					...
					<br>
					<p style="text-align: right;">개인정보 수집 및 이용에 동의하십니까?<input type="checkbox" id="tos2">동의함</p>
				</div><!-- foot_body_textarea -->
			</div><!-- foot_body -->
			<p align="center">
